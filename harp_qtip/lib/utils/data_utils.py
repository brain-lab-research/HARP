...
def unpack_quip(module, saved_layer):
    ...
    if module.tlut is not None and saved_layer.get('tlut', None) is not None:
        module.tlut.copy_(saved_layer['tlut'].float().to(torch.float16))

    if 'rcp' in saved_layer:
        rcp = int(saved_layer['rcp'])
        SU = saved_layer['SU'].float()
        SV = saved_layer['SV'].float()
        Wscale = saved_layer['Wscale'].float()
        tp_rank = int(saved_layer['tp_rank'])

        module.rcp.copy_(torch.tensor(rcp))
        module.tp_rank.copy_(torch.tensor(tp_rank))

        if rcp == 1:
            module.SU.copy_(
                (SU.reshape(tp_rank, -1) * Wscale.unsqueeze(-1)).reshape(SU.shape))
            module.SV.copy_(SV)
        elif rcp == 2:
            module.SU.copy_(SU)
            module.SV.copy_(
                (SV.reshape(tp_rank, -1) * Wscale.unsqueeze(-1)).reshape(SV.shape))
        else:
            module.SU.copy_(SU)
    ...
    incoh_mode = saved_layer.get('incoh_mode', 'had')
    module.incoh_mode = incoh_mode

    if incoh_mode == 'harp':
        from lib.incoherence.harp import HARPProcessor

        harp_cfg = dict(saved_layer.get('harp_cfg', {}) or {})
        module.harp_cfg = harp_cfg

        rcp_int = int(saved_layer.get('rcp', 0))
        tp_rank = int(saved_layer.get('tp_rank', 1))
        rcp_name = None
        if rcp_int == 1:
            rcp_name = 'row'
        elif rcp_int == 2:
        ...
        split_for_tp = bool(rcp_name is not None and tp_rank > 1)

        module.harp = HARPProcessor(
            n=module.in_features,
            m=module.out_features,
            device=str(module.SU.device),
            split_for_tp=split_for_tp,
            rcp=rcp_name if split_for_tp else None,
            tp_rank=tp_rank,
            td_x=module.td_x,
            td_y=module.td_y,
            L=module.L,
            K=module.K,
            V=module.V,
            tlut_bits=module.tlut_bits,
            decode_mode=module.decode_mode,
            for_kernel=module.has_kernel,
            **harp_cfg,
        )
        module.harp.to(module.SU.device)
        module.harp.load_state_dict(saved_layer['harp_state'], strict=True)
    else:
        module.harp_cfg = None
        module.harp = None
