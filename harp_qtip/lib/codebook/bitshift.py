...
class BitshiftLinear(nn.Module):
    ...
    def cache_hatW(self, packed_trellis, had_left, had_right, K_left, K_right,
               m, n, rcp, tp_rank, **kwargs):
        incoh_mode = kwargs.get("incoh_mode", "had")
        ...
        if incoh_mode == "harp":
            ...
            # IMPORTANT: for HARP, cache the weight in the rotated basis directly.
            # Do NOT Hadamard-manifest it.
            self.hatW = (hatW.float() / self.scale).to(self.internal_dtype).contiguous()
            return

        # Original QTIP / Hadamard path
        if self.has_kernel:
            hatW = self.get_hatW_kernel(packed_trellis, m, n)
        else:
            hatW = self.get_hatW(
    ...
    def forward(self,
        ...
        if incoh_mode == "harp":
            if harp is None:
                raise RuntimeError("incoh_mode='harp' but harp module is None")

            x = input.view(-1, n).to(torch.float32)
            x = x * SU.to(x.device, dtype=x.dtype)

            # x_tilde = x V
            x = harp.forward_pre_transform(x.contiguous()).to(torch.float32).contiguous()

            if mode == 'train-fixW':
                y_tilde = (x.to(self.internal_dtype) @ self.hatW.T).to(torch.float32)

            else:
                bs = x.shape[0]
                x_scaled = (x / self.scale).contiguous()

                if bs == 1 and self.has_kernel:
                    wrapper = getattr(
                        torch.ops.quip_lib,
                        f"decompress_matvec_qtip_{m}_1_{x_scaled.numel()}_{self.cb.K}"
                    )
                    # IMPORTANT: upcast to fp32 before post-transform
                    y_tilde = wrapper(
                        trellis,
                        x_scaled.contiguous(),
                        self.cb.tlut
                    ...
                    if self.has_kernel:
                        # IMPORTANT: upcast to fp32 before post-transform
                        y_tilde = BitshiftLinearKernelAG.apply(
                            x_scaled,
                            trellis,
                            m,
                            n,
                        ...
                        if mode == 'eval':
                            trellis_local = self.cb.unpack_trellis(
                                trellis, self.td_x * self.td_y
                            )
                        else:
                            trellis_local = trellis

                        hatW = self.get_hatW(trellis_local, m, n)
                        y_tilde = (x_scaled.to(hatW.dtype) @ hatW.T).to(torch.float32)

                # y = y_tilde U^T
                y = harp.forward_post_transform(y_tilde.contiguous()).to(torch.float32)

            y = y.to(SV.device) * (SV.to(y.device, dtype=y.dtype) * self.scale)
            return y.view(*input.shape[:-1], m).to(input.dtype)

        # ------------------------------------------------------------------
        # Original QTIP / Hadamard runtime path
        # ------------------------------------------------------------------
        x = input.view(-1, n).to(torch.float32)
        x = x * SU
            ...
            if bs == 1 and self.has_kernel:
                ...
                if self.has_kernel:
                    x = BitshiftLinearKernelAG.apply(
                        x, trellis, m, n, self.cb.L, self.cb.tlut_bits,
                        self.cb.K, self.V, self.cb.lut
                    ).float()
                else:
                    if mode == 'eval':
