...
class QuantizedLinear(nn.Module):
    ...
    def __init__(
        ...
        self.split_for_tp = bool(split_for_tp)
        self.rcp_name = rcp
        self.tp_rank_int = int(tp_rank)
        ...
        if self.incoh_mode == 'harp':
            ...
            self.harp = HARPProcessor(
                ...
                m=out_features,
                device="cpu",
                split_for_tp=self.split_for_tp,
                rcp=self.rcp_name if self.split_for_tp else None,
                tp_rank=self.tp_rank_int,
                td_x=self.td_x,
                td_y=self.td_y,
                L=self.L,
                K=self.K,
    ...
    def no_ckpt_forward(self, input):
        if not self.built_codebook_class:
            ...
            if self.mode == 'eval':
                ...
                self.codebook_class.cache_hatW(
                    self.trellis,
                    self.had_left,
                    self.had_right,
                    self.K_left,
