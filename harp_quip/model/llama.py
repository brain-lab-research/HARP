...
def _is_quant_layer(config: LlamaConfig, layer_idx: Optional[int]) -> bool:
    """
    True if this layer should use quip quant modules.
    - If config.quip_params missing => treat as fully dense (False)
    - If quip_params['quant_layers'] is None/missing => all layers quantized (True)
    - Else quant_layers is a list of indices to quantize
    """
    qp = getattr(config, "quip_params", None)
    if not isinstance(qp, dict):
        return False

    ql = qp.get("quant_layers", None)
    if ql is None:
        return True
    ...
    return int(layer_idx) in {int(x) for x in ql}
...
class LlamaRotaryEmbedding(nn.Module):
    def __init__(
        ...
        self.rope_kwargs = {}
        if config is None:
            # Backward-compatible path (kept, but for Llama3 you should pass config)
            logger.warning_once(
                "`LlamaRotaryEmbedding` should be initialized with `config=` for newer Llama models."
            )
            self.rope_kwargs = {
                "rope_type": rope_type,
            ...
            # Newer configs: rope_scaling may contain rope_type/type/etc.
...
class LlamaLinearScalingRotaryEmbedding(LlamaRotaryEmbedding):
    def __init__(self, *args, **kwargs):
        logger.warning_once(
            "`LlamaLinearScalingRotaryEmbedding` is deprecated. Use `LlamaRotaryEmbedding(config=...)`."
        )
...
class LlamaDynamicNTKScalingRotaryEmbedding(LlamaRotaryEmbedding):
    def __init__(self, *args, **kwargs):
        logger.warning_once(
            "`LlamaDynamicNTKScalingRotaryEmbedding` is deprecated. Use `LlamaRotaryEmbedding(config=...)`."
        )
...
class LlamaMLP(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.is_quant = _is_quant_layer(config, layer_idx)
        ...
        if self.is_quant:
            self.upgate_proj = FusedQuantizedLinear(
                ...
                incoh_mode=config.quip_params.get('incoh_mode', 'had'),
                harp_cfg=config.quip_params.get('harp', None),
            )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.is_quant:
            ...
            out = self.down_proj(self.act_fn(gate_proj) * up_proj)
...
class LlamaAttention(nn.Module):
    ...
    def _project_qkv(self, hidden_states: torch.Tensor):
        if self.is_quant:
            q, k, v = self.qkv_proj(hidden_states.to(torch.float32))
...
class LlamaFlashAttention2(LlamaAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_output = self.o_proj(attn_output).to(query_states.dtype)
