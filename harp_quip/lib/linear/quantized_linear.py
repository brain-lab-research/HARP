...
from lib.utils import clean, dtype_from_str, get_hadK, pop_explicit_kwargs
...
class QuantizedLinear(nn.Module):
    ...
    def __init__(
        ...
        self.incoh_mode = incoh_mode
        self.harp_cfg = harp_cfg
        self.harp = None
        if self.incoh_mode == "harp":
            ...
            self.harp = HARPProcessor(
                n=in_features, m=out_features,
                device="cpu",
                **harp_kwargs,
            )
    ...
    def no_ckpt_forward(self, input):
        if not self.built_codebook_class:
            ...
            if self.train_mode:
                had_left_T_T = self.had_left_T
                if had_left_T_T is not None:
                    had_left_T_T = had_left_T_T.T.contiguous()
