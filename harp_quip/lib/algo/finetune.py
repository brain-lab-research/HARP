...
def finetune_decoder_layer(layer, name, device, train_dl, valid_dl, args):
    ...
    susv_params, harp_params, other_params = utils.extract_susv_params(layer)
    optim = utils.get_susv_adam(susv_params, harp_params, other_params, args)
...
def quantize_finetune_decoder_layer(mixed_layer, quant_order, idx, cb, args,
    ...
    if do_ft:
        if pre_orig_emb is None or orig_emb is None:
            raise ValueError("pre_orig_emb/orig_emb must be provided when ft_epochs > 0")
    ...
    for quant_i, (linear_attr, name) in enumerate(quant_order):
        ...
        module_calibration_times_s[name] = module_elapsed_s
        glog.info(
        ...
        quant_linear.SV = nn.Parameter(quant_linear.SV.float(),
                                       requires_grad=True)
        if getattr(quant_linear, "incoh_mode", None) == "harp" and getattr(
                quant_linear, "harp", None) is not None:
            for p in quant_linear.harp.parameters():
                p.requires_grad_(args.ft_harp_lr > 0)
        ...
        # Skip all finetuning-related work when ft_epochs == 0
        if do_ft and quant_i < len(quant_order) - 1:
    ...
    glog.info(
        f'layer {idx} whole layer-block calibration time: {layer_block_calibration_time_s:.2f}s'
    )
...
def finetune_susv_e2e(model, orig_logits, emb, position_ids, attention_mask,
    ...
    for name, module in model.named_modules():
        if isinstance(module, QuantizedLinear) or isinstance(
            ...
            if getattr(module, "incoh_mode", None) == "harp" and getattr(module, "harp", None) is not None:
                for p in module.harp.parameters():
                    p.requires_grad_(args.ft_harp_lr > 0)
    model.float()

    train_dl, valid_dl = utils.split_data(emb, orig_logits, args)

    susv_params, harp_params, other_params = utils.extract_susv_params(model)
    optim = utils.get_susv_adam(susv_params, harp_params, other_params, args)
