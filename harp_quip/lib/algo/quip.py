...
from lib.utils import pop_explicit_kwargs
...
def _safe_cholesky(H: torch.Tensor,
                   base_jitter: float = 1e-8,
                   max_tries: int = 6):
    # restore exact symmetry lost in floating point
    H = 0.5 * (H + H.T)

    diag_scale = H.diagonal().abs().mean().clamp_min(torch.finfo(H.dtype).eps)
    I = torch.eye(H.shape[0], device=H.device, dtype=H.dtype)

    # first try without jitter, then increase jitter geometrically
    for i in range(max_tries):
        if i == 0:
            H_try = H
            jitter = 0.0
        else:
            jitter = (10.0 ** (i - 1)) * base_jitter * diag_scale
            H_try = H + jitter * I

        L, info = torch.linalg.cholesky_ex(H_try)
        if int(info.item()) == 0 and torch.isfinite(L).all():
            return L, H_try, jitter

    return None, H, None


def incoherence_preprocess(H, W, args, cb=None, x_samples=None):
    ...
    if args.incoh_mode == "had":
        ...
        Hr = RHT_H(Hr, SU)
        Wr = RHT_W(Wr, SU, SV)
        incoh = {'mode': 'had', 'SU': SU.cpu(), 'SV': SV.cpu()}
    ...
    # randomized kronecker product on H, W
        ...
        Hr = SU @ Hr @ SU.T
        Wr = SV @ Wr @ SU.T
        incoh = {'mode': 'kron', 'SU': SU.cpu(), 'SV': SV.cpu()}
    elif args.incoh_mode == "harp":
        from lib.incoherence.harp import HARPProcessor

        if cb is None:
            raise ValueError("HARP requires codebook `cb` for proxy quantization.")

        harp_cfg = getattr(args, "harp_cfg", None)
        if harp_cfg is None:
            raise ValueError(
                "args.harp_cfg is None."
            )
        
        explicit = {
                "n", "m", "device",
            }
        harp_kwargs = pop_explicit_kwargs(harp_cfg, explicit)
        ...
        if x_samples is not None:
            xs = x_samples.to(device=device, dtype=torch.float32)
            if args.rescale_WH and scaleWH is not None:
                xs = xs / scaleWH.to(device=device, dtype=torch.float32)
            xs = xs * SU[None, :]

        Hr = Hr * SU[None, :] * SU[:, None]
        Wr = Wr * SV[:, None] * SU[None, :]

        cb = cb.to(device=device)

        torch.cuda.empty_cache()
        with torch.enable_grad():
            harp.fit(Hr, Wr, x_samples=xs, codebook=cb, args=args)

        Hr, Wr = harp.transform(Hr, Wr)

        incoh = {
            'mode': 'harp',
            'SU': SU.cpu(),
            'SV': SV.cpu(),
            'harp_cfg': dict(harp_cfg),
            'harp_state': {k: v.detach().cpu() for k, v in harp.state_dict().items()},
        }
    else:
        raise NotImplementedError

    Lhr, Hr, jitter = _safe_cholesky(Hr)
    if Lhr is None:
        glog.warning(
            "incoherence_preprocess: Cholesky failed after preprocessing; "
            "returning None so quantize() can retry in fp64."
        )
        return None

    if jitter is not None and jitter > 0:
        glog.warning(f"incoherence_preprocess: added jitter {jitter:.3e} before Cholesky.")

    Wr = Wr.to(device)
    return Lhr, Hr, Wr, incoh, scaleWH


def incoherence_process(hatWr, incoh, scaleWH, args):
    device = hatWr.device
    SU, SV = incoh['SU'], incoh['SV']
    mode = incoh['mode']
    ...
    # reverse kronecker product
        ...
        harp_kwargs = pop_explicit_kwargs(cfg, explicit)
        ...
        harp.load_state_dict(incoh['harp_state'], strict=True)
        hatWr = harp.inverse_transform_weights(hatWr)
        hatWr = hatWr * SV.to(device)[:, None]
        hatWr = hatWr * SU.to(device)[None, :]
    else:
        raise NotImplementedError
...
def quantize(H_orig, W_orig, rank, codebook_orig, args, device='cpu', x_samples=None):
    ...
    incoh_out = incoherence_preprocess(H, W, args, cb=codebook, x_samples=x_samples)
    ...
    Lhr, Hr, Wr, incoh, scaleWH = incoh_out
    SU, SV = incoh['SU'], incoh['SV']
    del incoh_out
    utils.clean()
    ...
    hatW = incoherence_process(hatWr, incoh, scaleWH, args)
    ...
    attr = {
        ...
        'scaleWH': scaleWH,
        'incoh_mode': incoh['mode'],
    }
    if incoh['mode'] == 'harp':
        attr['harp_cfg'] = incoh['harp_cfg']
        attr['harp_state'] = incoh['harp_state']
...
def quantize_linear(weights, save_path, hessian_path, cb, args, device='cpu'):
    ...
    x_samples = None
    if getattr(args, "incoh_mode", "had") == "harp":
        if 'x_samples' in H_data:
            x_samples = H_data['x_samples']
        else:
            xs_dir = getattr(args, "xsamples_path", None)
            if xs_dir is not None:
                xs_path = os.path.join(xs_dir, os.path.basename(hessian_path))
                if os.path.exists(xs_path):
                    xs_data = torch.load(xs_path, map_location=torch.device('cpu'))
                    x_samples = xs_data['x_samples']
        # move to quantization device
        if x_samples is not None:
            x_samples = x_samples.to(dtype_).to(device)
    ...
    hatW, attr = quantize(H, W, args.lora_rank, cb, args, device, x_samples=x_samples)
    if len(scales) == 1:
