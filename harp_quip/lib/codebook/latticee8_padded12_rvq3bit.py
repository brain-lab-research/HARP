...
class QuantizedE8P12RVQ3BLinear(nn.Module):
    ...
    def cache_WH(self,
        ...
        resid_scale = resid_scale_override if resid_scale_override > 0 else self.codebook.opt_resid_scale
        ...
        if incoh_mode == "harp":
            ...
            W_full = W_decompressed + W_resid_decompressed.float() / resid_scale
            self.W = (W_full / self.scale).T.to(torch.float16)   # [n, m]
            return
    ...
    def forward(self,
        ...
        if incoh_mode == "harp":
            ...
            x = harp.forward_pre_transform(x)

            resid_scale = resid_scale_override if resid_scale_override > 0 else self.codebook.opt_resid_scale

            if train_mode:
                y_tilde = (x.to(torch.float16) @ self.W).float()
            else:
                x = x / self.scale
                ...
                if x.shape[0] == 1:
                    ...
                    z = torch.zeros(8, m, dtype=torch.float32, device=x16.device)
                    quiptools_cuda.lookupmatmul_e81b_k8(
                        x_padded / resid_scale,
                        Qidxs_list[1],
                        self.codebook.e81b_grid,
                        z,
                    )
                    ...
                    W_full = W_decompressed + W_resid_decompressed / resid_scale
                    y_tilde = (x16 @ W_full.T).to(torch.float32)

            y = harp.forward_post_transform(y_tilde)
            y = y * SV * self.scale
            return y.view(*input.shape[:-1], m)
