...
from lib.utils.misc import pop_explicit_kwargs
...
def unpack_quip(module, saved_layer, codebook_id, codesz):
    ...
    if saved_layer.get('incoh_mode', None) == 'harp':
        module.incoh_mode = 'harp'
        module.harp_cfg = saved_layer.get('harp_cfg', None)

        if getattr(module, "harp", None) is None:
            from lib.incoherence.harp import HARPProcessor
            cfg = module.harp_cfg or {}
            dev = module.SU.device
            ...
            module.harp = HARPProcessor(
                n=module.in_features, m=module.out_features,
                device=str(dev),
                **harp_kwargs
            )
            module.harp.to(dev)
